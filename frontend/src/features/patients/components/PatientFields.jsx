...
export async function getDuplicatePatientByPhone(value, currentPatientId = null) {
  const phone = String(value || '').trim()
  const formatError = validatePhone(phone)

  if (!phone || formatError) {
    return null
  }

  const response = await getPatients({ phone })
  const matchingPatients = normalizeList(response)

  return (
    matchingPatients.find(
      (patient) =>
        patient.phone === phone &&
...
export async function validateUniquePatientPhone(value, currentPatientId = null) {
  const duplicate = await getDuplicatePatientByPhone(value, currentPatientId)

  return duplicate ? 'Phone already registered' : null
}
